/**
 * Claim Ground — Epistemic Pushback Hook (OpenClaw mirror)
 *
 * Mirrors skills/claim-ground/hooks/epistemic-pushback-trigger.sh logic for
 * OpenClaw `message:received` event handlers. When user pushback regex is hit,
 * emit <CLAIM_GROUND_ACTIVATED> reminder requiring re-verification.
 */
...
function findMatchersFile(): string | null {
  const candidates = [
...
const claimGroundEpistemicPushback = async (
  event: any,
): Promise<{ injectContext?: string } | void> => {
