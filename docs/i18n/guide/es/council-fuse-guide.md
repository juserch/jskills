# Guia del usuario de Council Fuse

> Empieza en 5 minutos — deliberacion multiperspectiva para mejores respuestas
...
> **Cero dependencias** — Council Fuse no requiere servicios externos ni APIs. Instala y listo.
...
|---------|----------|---------------|
| `/council-fuse <pregunta>` | Ejecuta una deliberacion completa del consejo | Decisiones importantes, preguntas complejas |

---

## Como funciona

Council Fuse condensa el patron LLM Council de Karpathy en un solo comando:

### Etapa 1: Convocatoria

Se generan tres agentes **en paralelo**, cada uno con una perspectiva diferente:
...
El Presidente (agente principal) anonimiza todas las respuestas como Respuesta A/B/C y luego puntua cada una en 4 dimensiones (0-10):

- **Correccion** — precision factual, solidez logica
- **Completitud** — cobertura de todos los aspectos
- **Practicidad** — aplicabilidad, relevancia en el mundo real
...
El generalista ofrece un analisis equilibrado de compensaciones, el critico cuestiona el hype de los microservicios y el especialista detalla los patrones de migracion. La sintesis proporciona una recomendacion condicional.
...
Tres angulos diferentes aseguran que no pases por alto las preocupaciones operativas (critico), los detalles de implementacion (especialista) o la opcion pragmatica por defecto (generalista).
...
2. **Analisis de consenso** — donde coinciden y donde difieren
3. **Respuesta sintetizada** — la mejor respuesta fusionada
4. **Opinion minoritaria** — opiniones disidentes validas que vale la pena considerar
...
- `model: opus` — todos los pesos pesados para decisiones criticas
...
|------------|----------------------------------------|
| Claude Code | 3 spawns independientes de Agent en paralelo |
| OpenClaw | Un solo agente, 3 rondas de razonamiento independiente secuencial |

---

## Preguntas frecuentes

**P: Consume 3 veces los tokens?**
