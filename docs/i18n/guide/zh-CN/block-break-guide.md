...
> **状态不会重置** — 压力等级通过 `~/.forge/block-break-state.json` 持久化。上下文压缩、会话中断都不会让失败计数归零。逃不掉的。

### Hooks 配置

通过 `claude plugin add juserai/forge` 安装时，hooks 会自动配置。Hook 脚本需要 `jq`（首选）或 `python` 作为 JSON 引擎——你的系统中至少需要安装其中一个。

如果 hooks 没有触发，检查配置：

```bash
cat ~/.claude/settings.json  # 应包含引用 forge 插件的 hooks 条目
```

### 状态过期

状态在 **2 小时**无活动后自动过期。这可以防止前一次调试会话的陈旧压力影响到无关的工作。2 小时后，会话恢复 hook 会静默跳过恢复，你将从 L0 重新开始。

手动重置：`rm ~/.forge/block-break-state.json`
...
| 问题 | 原因 | 解决方法 |
|------|------|---------|
| Hooks 不自动触发 | 插件未安装或 hooks 未写入 settings.json | 重新运行 `claude plugin add juserai/forge` |
| 状态未持久化 | 系统中没有 `jq` 或 `python` | 安装其中一个：`apt install jq` 或确保 `python` 在 PATH 中 |
| 压力一直卡在 L4 | 状态文件累积了过多失败记录 | 重置：`rm ~/.forge/block-break-state.json` |
| 会话恢复显示旧状态 | 状态距上次会话不到 2 小时 | 这是预期行为；等 2 小时或手动重置 |
| `/block-break` 无法识别 | 当前会话未加载 skill | 重新安装插件或使用通用单行安装 |
...
删除状态文件即可：`rm ~/.forge/block-break-state.json`。或者等 2 小时——状态会自动过期（参见[状态过期](#状态过期)章节）。
